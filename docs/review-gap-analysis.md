...
| 採点回数 | **5回** | `freeTrialUsageLimit` デフォルト **5** (`AuthProvider.tsx:119`, `admin/page.tsx:48`) | **一致** |
| 自動課金 | なし | Stripe連携だが自動課金ロジックなし | 一致 |

**解決済み**: コードのデフォルト値を**5回**に統一。pricing/page.tsxのFAQセクションも「5回分の無料採点」に修正済み。
- `TrialEndedModal.tsx:68`: `{systemSettings?.freeTrialUsageLimit || 5}回`
- `FreeAccessBanner.tsx:71`: `systemSettings?.freeTrialUsageLimit || 5`
- `UsageStatus.tsx:46`: `systemSettings?.freeTrialUsageLimit || 5`

> 注: admin画面で `free_trial_usage_limit` を変更可能。デフォルトは5。
...
| 項目 | FAQ記載 | コードの実装 | 一致 |
|------|---------|-------------|------|
| ライト | ¥480/月 | ¥480/月（期間限定価格） | 一致 |
| スタンダード | ¥980/月 | ¥980/月（期間限定価格） | 一致 |
| 無制限 | ¥1,580/月 | ¥1,580/月（期間限定価格） | 一致 |

**注意**: FAQの価格は期間限定価格。コードには通常価格（ライト¥980、スタンダード¥1,980、無制限¥4,980）も存在し、取り消し線で表示される。
...
| 項目 | ガイド記載 | コードの実装 | 一致 |
|------|-----------|-------------|------|
| トライアル回数デフォルト | **5回** (`system_settings`テーブル) | **5回** (コードのフォールバック値) | **一致** |
| 管理ダッシュボード | `/admin` パスで記載 | `web/src/app/admin/page.tsx` が存在 | 一致 |
| 環境変数 | 10個記載 | 基本一致（GEMINI_API_KEY等） | 一致 |
| system_settingsテーブル | `free_trial_usage_limit` デフォルト5 | コードのフォールバックは5 | **一致** |
...
| # | 項目 | ドキュメント | コード | 影響 |
|---|------|-------------|--------|------|
| 1 | **無料体験回数** | 5回（FAQ, ガイド, 管理者ガイド） | 5回（デフォルト） | **解決済み** |
...
1. **~~無料体験回数を「3回」に修正~~** → **解決済み**: コードを5回に統一
