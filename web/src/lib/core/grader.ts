...
type RequiredPhraseCheckResult = {
    condition: string;
...
type MandatoryChecks = {
    ...
    required_phrase_check?: RequiredPhraseCheckResult;
...
const SMALL_KANA_PATTERN = /[ぁぃぅぇぉゃゅょっゎァィゥェォャュョッヮ]/g;
...
export class TaskalGrader {
    ...
    private readonly ocrSystemInstruction = [
        ...
        "",
        "【絶対厳守ルール - 違反は許容されません】",
        "1. 画像内の文字を一字一句そのまま転写する",
        "2. 要約・省略・補完・校正は厳禁（1文字でも省略したら失敗）",
        "3. 文字数が多くても省略せず全文を出力する",
        "4. 判読不能な文字のみ「〓」に置換",
        "",
        "【読み取りルール】",
        "- 縦書きは右から左、上から下に読む",
        "- 小書き仮名（ゃ・ゅ・ょ・っ・ぁ・ぃ・ぅ・ぇ・ぉ）は必ず小さい文字のまま転写する",
        "- 拗音を通常仮名に直さない（例: 「きょう」を「きよう」、「しゅ」を「しゆ」としない）",
        "- 小書き仮名を省略しない（例: 「しょう」を「しう」、「じゅん」を「じん」としない）",
        "- 「〜ので」「〜ため」「〜から」などの接続表現を省略しない",
    ...
    private async generateGradingResponseText(
        ...
        if (rateLimitManager.isRateLimited() && CONFIG.RATE_LIMIT_FALLBACK_MODEL) {
            ...
            console.info(`[Grader] レート制限中のため、${operationLabel}にフォールバックモデルを使用: ${gradingModel}`);
        }
        ...
        try {
            ...
            if (RateLimitManager.isRateLimitError(error) && CONFIG.RATE_LIMIT_FALLBACK_MODEL && gradingModel !== CONFIG.RATE_LIMIT_FALLBACK_MODEL) {
                ...
                console.warn(`[Grader] ${operationLabel}でレート制限エラー検出。${CONFIG.RATE_LIMIT_FALLBACK_MODEL} で再試行します。`);
                result = await withTimeout(
                    this.ai.models.generateContent({
    ...
    private ensureGradingResultObject(parsed: Record<string, unknown>): Record<string, unknown> {
        return (parsed.grading_result && typeof parsed.grading_result === "object")
            ? parsed.grading_result as Record<string, unknown>
            : (parsed.grading_result = {} as Record<string, unknown>);
    }

    private applyRecognizedTextPatch(parsed: Record<string, unknown>, patch: RecognizedTextPatch): void {
        const gradingResultObj = this.ensureGradingResultObject(parsed);
        gradingResultObj.recognized_text = patch.text;
        gradingResultObj.recognized_text_full = patch.fullText ?? patch.text;

        if (patch.source) {
            gradingResultObj.recognized_text_source = patch.source;
        }

        if (typeof patch.userConfirmed === "boolean") {
            gradingResultObj.user_confirmed = patch.userConfirmed;
        }
    }
    ...
    private countSmallKana(text: string): number {
        return text.match(SMALL_KANA_PATTERN)?.length ?? 0;
    }

    private selectBestRecognizedText<T extends { source: string; text: string }>(candidates: T[]): T | null {
        const validCandidates = candidates.filter(candidate => {
            const text = candidate.text.trim();
            return text && !OCR_PLACEHOLDER_PATTERN.test(text);
        });
        ...
        for (const candidate of validCandidates.slice(1)) {
            const bestLength = this.compactText(best.text).length;
            const candidateLength = this.compactText(candidate.text).length;
            const candidateIsSignificantlyLonger = candidateLength > bestLength * 1.2;

            if (candidateIsSignificantlyLonger) {
                best = candidate;
                continue;
            }

            // Gemini 3.5 Flash で小書き仮名が通常仮名へ戻されることがあるため、
            // 文字数が同程度なら小書き仮名を多く保持している候補を優先する。
            const isComparableLength = candidateLength >= bestLength * 0.8 && candidateLength <= bestLength * 1.2;
            if (isComparableLength && this.countSmallKana(candidate.text) > this.countSmallKana(best.text)) {
                best = candidate;
            }
        }
    ...
    private normalizeForRequiredPhraseCheck(text: string): string {
        return text
            .normalize("NFKC")
            .replace(/\s+/g, "")
            .toLowerCase();
    ...
    private addRequiredPhrase(phrases: Set<string>, raw: string): void {
        const cleaned = raw
            .replace(/^[「『"“”'`｢\s]+|[」』"“”'`｣\s]+$/g, "")
            .replace(/^(次の|以下の)/, "")
            .replace(/(という)?(語句|言葉|単語|表現|キーワード|文言|フレーズ)$/, "")
            .trim();

        if (!cleaned) return;
        if (cleaned.length > 40) return;
        if (/^\d+$/.test(cleaned)) return;
        if (/^(指定)?(語句|言葉|単語|表現|キーワード|文言|フレーズ|条件)$/.test(cleaned)) return;
        if (/(字以上|字以内|字以下|字未満|文字以上|文字以内|文字以下|文字未満)/.test(cleaned)) return;

        phrases.add(cleaned);
    }

    private extractRequiredPhrases(problemCondition?: string): string[] {
        if (!problemCondition) return [];

        const condition = problemCondition.trim();
        if (!condition) return [];

        const hasPhraseRequirement = /(語句|言葉|単語|表現|キーワード|文言|フレーズ|入れ|含め|使|用い|用いる|使用)/.test(condition);
        if (!hasPhraseRequirement) return [];

        const phrases = new Set<string>();

        for (const match of condition.matchAll(/[「『"“”'`｢]([^」』"“”'`｣]{1,40})[」』"“”'`｣]/g)) {
            this.addRequiredPhrase(phrases, match[1]);
        }

        for (const match of condition.matchAll(/(?:語句|言葉|単語|表現|キーワード|文言|フレーズ)[^:：]*[:：]\s*([^。\n]+)/g)) {
            for (const part of match[1].split(/[、,，・/／]/)) {
                this.addRequiredPhrase(phrases, part);
            }
        }

        for (const match of condition.matchAll(/([^\s、。,.，]{1,30})を(?:必ず)?(?:入れ|含め|使|用い|使用)/g)) {
            this.addRequiredPhrase(phrases, match[1]);
        }
    ...
    private checkRequiredPhrasesProgrammatically(recognizedText: string, problemCondition?: string): RequiredPhraseCheckResult | undefined {
        const requiredPhrases = this.extractRequiredPhrases(problemCondition);
        if (requiredPhrases.length === 0 || !problemCondition) return undefined;

        const normalizedAnswer = this.normalizeForRequiredPhraseCheck(recognizedText);
        const missingPhrases = requiredPhrases.filter(phrase => {
            const normalizedPhrase = this.normalizeForRequiredPhraseCheck(phrase);
            return normalizedPhrase && !normalizedAnswer.includes(normalizedPhrase);
        });
        ...
        return {
            ...
            satisfied: missingPhrases.length === 0,
            deduction: missingPhrases.length > 0 ? 90 : 0
        };
    }

    private buildIncompleteGradingRepairPrompt(
        basePrompt: string,
...
【重要: 再生成】
直前の応答は不完全でした。欠けていた項目:
- ${missingFields.join("\n- ")}

以下の不完全なJSONを修正し、必須項目をすべて埋めた完全なJSONのみを返してください。
- status は必ず "success" にする
- grading_result.score は 0〜100 の整数で必須
- grading_result.feedback_content.good_point / improvement_advice / rewrite_example をすべて 5文字以上で埋める
- recognized_text は既存値を維持する
- deduction_details がある場合、score と整合させる
- JSON以外の説明文は一切出力しない
...
${JSON.stringify(partialResult, null, 2)}`;
    ...
    private async repairIncompleteGradingResult(params: {
        basePrompt: string;
        imageParts: ContentPart[];
        strictness: GradingStrictness;
        partialResult: Record<string, unknown>;
        missingFields: string[];
        recognizedTextPatch: RecognizedTextPatch;
        ...
        try {
            console.warn(`[Grader] ${logLabel}: 不完全応答を再生成します`, missingFields);
            const repairPrompt = this.buildIncompleteGradingRepairPrompt(basePrompt, partialResult, missingFields);
            const repairText = await this.generateGradingResponseText(
                repairPrompt,
                imageParts,
                strictness,
                `${logLabel}補完`
            ...
            console.log(`[Grader] ${logLabel}補完レスポンス長:`, repairText.length);
            const repairedParsed = this.extractJsonFromText(repairText);
            if (!repairedParsed) {
                console.error(`[Grader] ${logLabel}: 補完レスポンスのJSONパースに失敗`);
                return null;
            }

            delete repairedParsed.debug_info;
            this.applyRecognizedTextPatch(repairedParsed, recognizedTextPatch);

            const repairedValidated = this.validateAndEnhanceGrading(repairedParsed, problemCondition);
            const repairedQuality = this.validateGradingQuality(repairedValidated);
            if (!repairedQuality.isValid) {
                console.error(`[Grader] ${logLabel}: 補完後も必須フィールドが不足`, repairedQuality.missingFields);
                return null;
            }

            console.log(`[Grader] ✅ ${logLabel}: 補完再生成に成功`);
            return repairedValidated;
        } catch (error) {
            console.error(`[Grader] ${logLabel}: 補完再生成に失敗`, error);
            return null;
        }
    }
    ...
    private buildOcrPrompt(
        ...
        // 2025-01-28: 省略防止のため品質チェック項目を追加
        const cotPrompt = isShortAnswer
            ? `マス目OCR（短文記述問題）。${targetDescription}
${hasAllRole ? `手書きの答案部分のみ読み取り。印刷文字は無視。` : ""}${gridDescription}

【タスク - 省略厳禁】
1. まず全体を見渡し、およその総文字数を把握する
2. マス目を1つずつ順番に確認して文字を読み取る
3. 横書き: 左上→右→下、縦書き: 右上→下→左
4. 空マスは無視（文末の空白は除外）

【品質チェック - 必ず実行】
- 読み取った文字数と、目視で確認した総文字数がほぼ一致するか確認
- 小書き仮名（ゃ・ゅ・ょ・っ・ぁ・ぃ・ぅ・ぇ・ぉ）を通常仮名に戻していないか確認
- 拗音を省略していないか確認（例: 「きょう」「しょう」「じゅん」「ちゃ」を正確に保持）
- 「〜ので」「〜ため」「〜から」などの接続表現が途切れていないか確認
- 文が不自然に短くなっていないか確認
- 省略した可能性があれば、最初から読み直す
...
【出力JSON】
{
  ...
  "layout": {
    ...
    "indented_columns": [<字下げ列番号>]
        ...
        // 2025-01-28: リトライ時のプロンプトも省略防止を強調
        if (mode === "retry") {
            finalPrompt = `【再試行 - 省略厳禁】前回の読み取りで文字が省略された可能性があります。
今回は1文字ずつ慎重に確認し、絶対に省略しないでください。
特に「〜ので」「〜ため」「〜から」などの接続表現が抜けていないか注意。
小書き仮名（ゃ・ゅ・ょ・っ等）を通常仮名に戻したり省略したりしないでください。

` + cotPrompt;
        }

        if (mode === "detail") {
            finalPrompt = `【最終パス - 全文出力必須】これが最後の試行です。
1文字ずつ確認し、1文字も省略せずに全文を出力してください。
文字数が予想より少ない場合は、読み飛ばした部分がないか再確認。
小書き仮名（ゃ・ゅ・ょ・っ等）を通常仮名に戻したり省略したりしないでください。
...
JSONのみ出力してください。`;
     ...
     */
    private validateAndEnhanceGrading(parsed: Record<string, unknown>, problemCondition?: string): Record<string, unknown> {
        ...
        if (parsed.ocr_debug) {
            ...
            // 最も妥当なテキストを選択。文字数だけでなく、小書き仮名の保持も優先する。
            if (candidates.length > 0) {
                const best = this.selectBestRecognizedText(candidates) ?? candidates.reduce((a, b) => a.length > b.length ? a : b);
                const betterSmallKanaPreservation = this.countSmallKana(best.text) > this.countSmallKana(currentText);
                if (best.length > currentLength || needsRecovery || betterSmallKanaPreservation) {
        ...
        const requiredPhraseCheck = this.checkRequiredPhrasesProgrammatically(recognizedText, problemCondition);
        if (requiredPhraseCheck) {
            programmaticChecks.required_phrase_check = requiredPhraseCheck;
        }
        ...
        // 3. 指定語句チェック（ユーザー指定条件に「入れる・含める・使う」等がある場合）
        if (requiredPhraseCheck && !requiredPhraseCheck.satisfied) {
            deductionDetails = deductionDetails.filter(d =>
                !d.reason?.includes("指定語句") &&
                !d.reason?.includes("必須語句") &&
                !d.reason?.includes("キーワード不足")
            );

            const missing = requiredPhraseCheck.missing_phrases.map(phrase => `「${phrase}」`).join("、");
            console.log("[Grader] プログラム検証: 指定語句不足を検出、90%減点:", missing);
            deductionDetails.push({
                reason: `指定語句不足（${missing}を含んでいない）`,
...
※ 特定の語句・言葉・キーワードを入れる条件がある場合、recognized_text にその語句が完全に含まれるか確認してください。
※ 指定語句が含まれていない場合は、deduction_details に 90% 減点を必ず追加してください。
` : '';
...
- 指定語句・キーワードを入れる条件があり、その語句が答案に含まれない場合は 90% 減点する
- recognized_text は上記の確認済みテキストをそのまま出力すること
- status は必ず "success" にすること
- grading_result.score は 0〜100 の整数で必ず出力すること
- grading_result.feedback_content.good_point / improvement_advice / rewrite_example をすべて 5文字以上で必ず埋めること
${layout ? '- 【重要】上記のレイアウト情報を信頼し、字下げ・行数・段落構成での減点は禁止' : ''}

結果はJSON形式のみで出力してください。`;

        const text = await this.generateGradingResponseText(
            prompt,
            imageParts,
            strictness,
            "採点処理"
        ...
        if (parsed) {
            ...
            const recognizedTextPatch: RecognizedTextPatch = {
                text: confirmedText,
                fullText: confirmedText,
                userConfirmed: true,
            };
            this.applyRecognizedTextPatch(parsed, recognizedTextPatch);

            // プログラムによる検証・補完を実行
            const validated = this.validateAndEnhanceGrading(parsed, problemCondition);

            // 品質検証: 必須フィールドが揃っているかチェック
            const qualityResult = this.validateGradingQuality(validated);
            if (!qualityResult.isValid) {
                const repaired = await this.repairIncompleteGradingResult({
                    basePrompt: prompt,
                    imageParts,
                    strictness,
                    partialResult: validated,
                ...
                if (repaired) {
                    console.log("[Grader] 採点完了（確認済みテキスト使用・補完再生成）");
                    return repaired;
                }
...
- 句読点も1文字として数える
- 小書き仮名（ゃ・ゅ・ょ・っ・ぁ・ぃ・ぅ・ぇ・ぉ）は小さい文字のまま残す
- 拗音を通常仮名に直さない（例: 「きょう」を「きよう」、「しゅ」を「しゆ」としない）
- 読めない文字は「〓」で出力
- 推測や補完は禁止
...
特に小書き仮名（ゃ・ゅ・ょ・っ等）が、通常仮名へ置き換わっていないか、省略されていないかを確認してください。
...
` : `
【重要：指定語句条件の確認】
問題文に「次の語句を使って」「この言葉を入れて」「指定語句を含めて」等の条件がある場合、recognized_text にその語句が完全に含まれるか確認してください。
指定語句が含まれていない場合は、deduction_details に 90% 減点を必ず追加してください。
`;
...
${pdfPageHint}
${problemConditionSection}${modelAnswerSection}
${ocrSection}
...
結果はJSON形式のみで出力してください。`;
        ...
        if (parsed) {
            ...
            const selectedCandidate = this.selectBestRecognizedText(candidates);
            let finalRecognized = selectedCandidate?.text ?? "";
            let selectedSource = selectedCandidate?.source ?? "none";
            ...
            const recognizedTextPatch: RecognizedTextPatch = {
                text: finalRecognized,
                fullText: normalizedFull || finalRecognized,
                source: {
            ...
            if (!qualityResult.isValid) {
                ...
                if (repaired) {
                    console.log("[Grader] Stage 2 完了: 補完再生成により採点結果を復旧");
                    return repaired;
                }
