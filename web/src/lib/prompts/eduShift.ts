...
## ⚠️ 最重要警告: AIによる「解釈」「補完」「修正」は絶対禁止 ⚠️
...
2. **縦書き・マス目の厳密な読み取り（重要）:**
   ...
   - **各マスには1文字だけ入っています。1マスずつ確認してください。**
...
- **文字数カウント:** \`recognized_text\` の文字数を厳密にカウントしてください（空白や改行は除く）。指定字数がある場合、このカウントに基づいて判定してください。見た目の空白ではなく、実際に書かれた文字数を基準とします。
  - **8割ルール:** 指定文字数の8割未満の場合は、大幅減点（得点率50%カット）または0点として扱ってください。
  - **文字数超過:** 指定字数を超えた場合は0点として扱ってください。
