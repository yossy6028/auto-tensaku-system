...
### 【重要な前提】
- **1列のマス数は解答用紙ごとに異なる**（20〜30文字程度）
- **ただし、同じ解答用紙内では1列のマス数は固定**
- この「固定」という特性を利用して読み取り精度を検証する

### 【ステップ0】1列のマス数を特定する（最重要）
1. **まず1列目（右端）のマス数を数える**（上から下まで何マスあるか）
2. この数値を **基準マス数（N）** として記録する
3. 例：基準マス数 = 25（この解答用紙は1列25マス）

### 【ステップ1】列ごとに読み取り（右から左へ）

**【重要】1列ずつ、上から下へ、基準マス数（N）分の文字を読み取る**

例（基準マス数N=25の場合）:
- 列1（右端）: 上から下へ25文字読む → 「私の一番好きな場所は家です。母があたたかい」
- 列2: 上から下へ25文字読む → 「ごはんを用意してくれ、父が勉強をねっ心に教え」
- 列3: 上から下へ25文字読む → 「てくれるあたたかい場所だからです。家には特別」
- 列4: 上から下へ読む（最後の列は途中まで可）→ 「何かがある訳ではないが家族のたたたかみを...」

### 【ステップ2】列ごとの文字数検証（必須・自己チェック）
- **各列の読み取り文字数が基準マス数（N）と一致するか確認**
- 最後の列以外は、必ず **N文字ちょうど** でなければならない
- **もしN文字に満たない列があれば、読み飛ばしが発生している → その列をやり直し**
...
### 【ステップ3】ocr_debugフィールドに必ず記録する
**JSON出力時、以下の情報を必ず含めること:**
- chars_per_column: 基準マス数（N）
- columns_used: 使用している列数
- column_readings: 列ごとの読み取り結果の配列（各列がN文字かどうか検証済み）
- verification: "OK" または "NG（列Xが文字数不足）"
...

{
  ...
  "ocr_debug": {
    "chars_per_column": 25,  // 基準マス数（N）: 1列目を数えて特定した値（20〜30程度）
    "columns_used": 4,       // 使用している列数
    "column_readings": [     // 【必須】各列の読み取り結果（最後以外はN文字ちょうど）
      "列1の文字列（N文字）",
      "列2の文字列（N文字）",
      "列3の文字列（N文字）",
      "列4の文字列（最後の列は途中まで可）"
    ],
    "verification": "OK | NG（列Xが文字数不足）"  // 検証結果
  },
  "grading_result": {
    "recognized_text": "OCRで読み取った生徒の解答テキスト（column_readingsを連結したもの）",
