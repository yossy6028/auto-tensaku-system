...
### 3-C: 必須チェックリスト（全項目を必ず実行し、結果を出力せよ）

## 🔴 重要: 以下の5項目は「必ず」チェックし、結果をJSONに出力してください 🔴
## チェックを1つでも忘れると採点が無効になります

### ✅ チェック1: 文体統一（常体/敬体）
- 常体（だ・である調）: 「〜だ」「〜である」「〜と思う」「〜だから」
- 敬体（です・ます調）: 「〜です」「〜ます」「〜と思います」「〜ですから」
- **判定**: 混在あり → -10%、統一されている → 減点なし
- **出力必須**: style_check: { style: "常体" or "敬体" or "混在", examples: [...], deduction: 0 or 10 }

### ✅ チェック2: 語彙の重複
- 同じ単語・形容詞が何回出現するかカウント
- **判定**: 2回 → -5%、3回以上 → -10%
- **出力必須**: vocabulary_check: { repeated_words: [{word: "あたたかい", count: 3}], deduction: 5 or 10 }

### ✅ チェック3: 文字数（マス目ベース）
- マス目がある場合: マスの数で判定
- 8割未満: -50%、超過: 0点
- **出力必須**: character_count_check: { limit: 100, actual: 92, status: "OK" or "不足" or "超過", deduction: 0 }

### ✅ チェック4: 誤字・脱字
- 明らかな誤字・脱字をカウント
- **判定**: 1個につき -2%
- **出力必須**: typo_check: { typos: ["ねっ心→熱心?"], count: 1, deduction: 2 }

### ✅ チェック5: 内容の質（作文・自由記述のみ）
- 具体性、理由・根拠、独自の視点を評価
- **判定**: 不足があれば -5%〜-10%
- **出力必須**: content_check: { specificity: "OK/不足", reasoning: "OK/不足", originality: "OK/不足", deduction: 0 }
