...
## 🔴 絶対厳守: OCRは「2段階プロセス」で実行せよ 🔴

### 【第1段階】マス数カウント（採点の前に必ず実行）
1. 生徒の解答欄の画像を見る
2. **マスの総数**を数える（例：10行×8列=80マス）
3. **空白マス**を数える（何も書いていないマス）
4. **文字があるマス数** = 総マス数 - 空白マス数
5. この数字を記録する（例：72文字）

### 【第2段階】1マスずつ読み取り
- **記録した文字数と同じ数の文字を出力するまで終わらない**
- 縦書きの場合：右端の列から、上→下、次の列へ
- **1マス読んだら1文字出力**を72回（例）繰り返す
- 途中で省略しない、飛ばさない、まとめない

### 【検証】文字数が一致するか確認
- 出力した文字数 ≠ 第1段階で数えたマス数 → **やり直し**
- 一致するまで第2段階を繰り返す
...

{
  ...
  "grading_result": {
    "ocr_verification": {
      "total_squares": "解答欄のマス総数（例: 80）",
      "empty_squares": "空白マスの数（例: 8）",
      "expected_characters": "期待される文字数 = total - empty（例: 72）",
      "actual_characters": "実際に出力した文字数（例: 72）",
      "match": true | false
    },
    "recognized_text": "OCRで読み取った生徒の解答テキスト（1マス1文字、省略・変換なし）",
    "score": 0〜100の数値 (整数),
