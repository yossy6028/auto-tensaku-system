...
6. **複数の解答欄が横並びの場合（最重要）:**
   - 画像内に「問4」「問5」など複数の解答欄が**横に並んでいる**場合があります。
   - **指定された問題番号の解答欄のみ**を読み取ってください。隣の解答欄は絶対に読み取らないでください。
   - 各解答欄は「問○」というヘッダーや枠線で**明確に区切られた独立した領域**です。
   - 読み取り手順:
     1. まず画像全体を見て、指定された問題番号（例：問5）の解答欄の**左右の境界**を特定する
     2. その境界内のマス目**だけ**を、右上から縦に読み取る
     3. 隣の解答欄の文字が視界に入っても、**絶対に混ぜない**
...
- **表現の反復チェック（必須・減点5%）:** 同じ表現や似た表現が繰り返し使われていないか必ず確認してください。
  ...
  - ⚠️ **ただし、以下は減点対象外です:**
    - 助詞（「という」「ている」「ための」「ことで」など）
    - 助動詞（「れる」「られる」「ない」「た」など）
    - これらの付属語レベルの反復は自然な日本語表現であり、減点しないでください。
...
3. **比喩表現の扱い（重要）:**
   - 設問で「『〜のように』という比喩をふまえ」「比喩表現を説明せよ」などの指示がある場合、**比喩をそのまま答案に書いてはいけません**。
   - 「比喩をふまえ」とは「比喩を一般的な言い方に言い換えて説明せよ」という意味です。
   - 書き直し例では、比喩を具体的・一般的な表現に置き換えてください。
   - ❌ NG例: 「皆既日食の星空のように、普段は見えなくても二人には姉妹としての確かな絆があったということ。」（比喩をそのまま使用）
   - ✅ OK例: 「普段は表面に現れなくても、二人には姉妹としての確かな絆が存在していたということ。」（比喩を一般的な表現に言い換え）
   - 基本的に、答案に比喩表現を書くことは避けてください。
